...
def seed_database():
    ...
    session = Session()
    try:
        # Make sure tables exist before truncating
        session.execute(
            text(
                """
                TRUNCATE TABLE cancer_rates, cancer_ratios, uv_summary
                RESTART IDENTITY
                """
            )
        )
        ...
        cancer_rates_count = 0
        cancer_ratios_count = 0
        uv_summary_count = 0
        ...
        for _, row in df2.iterrows():
            ...
            cancer_ratios_count += 1
        session.commit()

        # This file is CSV-formatted even though it has .xls extension
        ...
        return {
            "cancer_rates_rows": cancer_rates_count,
            "cancer_ratios_rows": cancer_ratios_count,
            "uv_summary_rows": uv_summary_count,
        }
